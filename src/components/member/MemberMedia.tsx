...
type DocRow = { name: string; type: string; meta: string; href?: string };

export default function MemberMedia({ member }: { member: Member }) {
  const { getMemberLive } = useMessaging();
  const live = getMemberLive(member.id) ?? member;
  const portfolio = live.gallery ?? [];
  const resolved = useResolvedImages(portfolio);
  const lb = useLightbox(resolved.map((src, i) => ({ src, alt: `${live.name} — portfolio ${i + 1}` })));

  const { active } = useMemberProfile(member, 0);
  const documents = useMemo<DocRow[]>(() => {
    const rows: DocRow[] = [];
    const seen = new Set<string>();
    for (const d of member.sharedDocuments) {
      const key = `${d.type}:${d.name}`;
      if (seen.has(key)) continue;
      seen.add(key);
      rows.push({ name: d.name, type: d.type, meta: `Shared ${fmtDate(d.sharedAt)}` });
    }
    for (const o of active) {
      for (const d of o.documents ?? []) {
        ...
        if (seen.has(key)) continue;
        seen.add(key);
        rows.push({ name: d.name, type: d.type, meta: o.title, href: `/opportunity/${o.slug}` });
      }
    }
    return rows.slice(0, 8);
  }, [member.sharedDocuments, active]);

  if (portfolio.length === 0 && documents.length === 0) return null;

  const thumbs = portfolio.slice(1, 5);
  const extra = Math.max(0, portfolio.length - 5);
    ...
    <section>
      <div className="mb-5 flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1.5 text-xl md:text-2xl font-semibold text-navy-900 tracking-tight">Work &amp; documents</h2>
        </div>
        {portfolio.length > 0 ? (
          <span className="text-xs uppercase tracking-[0.14em] text-navy-700/60 font-semibold">{portfolio.length} images</span>
        ) : null}
      </div>
        ...
        {portfolio.length > 0 ? (
          <div>
            <button
              ...
              aria-label="Open portfolio image 1"
            >
              {resolved[0] ? (
                <Image src={resolved[0]} alt={`${live.name} — portfolio 1`} fill sizes="(min-width: 1024px) 640px, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-105" unoptimized />
              ) : (
              ...
              <div className="mt-2 grid grid-cols-4 gap-2">
                {thumbs.map((src, idx) => {
                  ...
                  return (
                    ...
                    >
                      {resolved[i] ? (
                        <Image src={resolved[i]} alt={`${live.name} — portfolio ${i + 1}`} fill sizes="160px" className="object-cover transition-transform duration-500 group-hover:scale-105" unoptimized />
                      ) : (
                        <div className="absolute inset-0 bg-navy-900/[0.06] animate-pulse" />
                      )}
                      {isLast ? (
                        <span className="absolute inset-0 bg-navy-900/70 flex items-center justify-center text-white text-sm font-semibold">+{extra}</span>
                      ) : null}
                    </button>
                  );
                })}
        ...
        <div className="rounded-2xl bg-white ring-1 ring-navy-900/[0.06] p-5">
          ...
          {documents.length > 0 ? (
                ...
                const inner = (
                  <>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-semibold text-navy-900 leading-snug truncate group-hover:text-gold-700 transition-colors">{d.name}</div>
                      <div className="text-[11px] text-navy-700/55">{d.type} · {d.meta}</div>
                    </div>
                    {d.href ? <ArrowUpRight className="h-4 w-4 text-navy-700/30 group-hover:text-gold-600 transition-colors shrink-0" strokeWidth={2.2} /> : null}
                  </>
                );
                return (
                  <li key={`${d.type}:${d.name}`}>
                    {d.href ? (
                      <Link href={d.href} className="group flex items-center gap-3 py-3 first:pt-0">{inner}</Link>
                    ) : (
                      <div className="flex items-center gap-3 py-3 first:pt-0">{inner}</div>
                    )}
                  </li>
                );
              })}
            ...
            <p className="mt-3 text-sm text-navy-700/60 leading-relaxed">
              Materials are shared through each opportunity&apos;s data room on request.
            </p>
          )}
        </div>
      </div>
