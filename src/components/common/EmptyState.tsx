...
/**
 * Shared empty-state: explains what's there, why, and offers a next action.
 * Used to standardize the "no data" panels across the platform.
 */
export default function EmptyState({
  ...
  const renderAction = (a: Action, primary: boolean) => {
    const cls = primary
      ? "inline-flex items-center justify-center gap-1.5 rounded-full bg-gold-500 hover:bg-gold-400 text-navy-900 font-semibold px-4 py-2 text-xs uppercase tracking-[0.12em] transition-colors"
      : "inline-flex items-center justify-center gap-1.5 rounded-full bg-white ring-1 ring-navy-900/15 hover:ring-navy-900/30 text-navy-900 font-semibold px-4 py-2 text-xs uppercase tracking-[0.12em] transition-colors";
    return a.href ? (
