...
import SelectionGrid, { type SelectionOption } from "@/components/wizard/SelectionGrid";
...
const LISTING_TYPE_OPTIONS: SelectionOption[] = [
  {
    value: "Opportunity",
    label: "Opportunity",
    description: "An investment, acquisition, partnership, or development project.",
    icon: Sparkles,
  },
  {
    value: "Product",
  ...
  },
  {
    value: "Service",
    label: "Service",
    description: "Professional services, advisory, construction, or supplier capacity.",
    icon: Wrench,
  },
];

const DEAL_TYPE_OPTIONS: SelectionOption[] = [
  { value: "Seeking Investor", label: "Seeking Investor", icon: HandCoins, description: "Equity or pref equity capital raise." },
  { value: "Joint Venture", label: "Joint Venture", icon: Handshake, description: "Operator + capital partner structure." },
  { value: "Land For Sale", label: "Land For Sale", icon: Mountain, description: "Titled land transaction." },
  { value: "Business For Sale", label: "Business For Sale", icon: Building, description: "Going-concern sale, founder transition." },
  { value: "Acquisition", label: "Acquisition", icon: Combine, description: "Buy-side mandate or platform deal." },
  { value: "Financing Needed", label: "Financing Needed", icon: PiggyBank, description: "Debt, mezz, or structured financing." },
  { value: "Supplier Offering", label: "Supplier Offering", icon: Truck, description: "Capacity, contracts, distribution." },
  { value: "Strategic Partnership", label: "Strategic Partnership", icon: Network, description: "Non-equity commercial alignment." },
  { value: "Development Project", label: "Development Project", icon: HardHat, description: "Ground-up or value-add build." },
  { value: "Franchise Opportunity", label: "Franchise Opportunity", icon: Store, description: "Multi-unit or single-territory rights." },
  { value: "Licensing Opportunity", label: "Licensing Opportunity", icon: KeyRound, description: "IP, brand, or technology license." },
  { value: "Service Offering", label: "Service Offering", icon: Briefcase, description: "Professional or B2B services." },
  { value: "Contact For Details", label: "Contact For Details", icon: MessageCircle, description: "Discreet listing — connect to learn more." },
];

const VALUE_TYPE_OPTIONS: SelectionOption[] = [
  { value: "Asking Price", label: "Asking Price", icon: Tag, description: "A fixed price you're asking from a buyer." },
  { value: "Seeking Investment", label: "Seeking Investment", icon: CircleDollarSign, description: "A capital amount you're raising." },
  { value: "Contact For Details", label: "Contact For Details", icon: MessageCircle, description: "Keep numbers private until a member connects." },
];
...
export default function CreateListingPage() {
    ...
    switch (step) {
      ...
      case 6:
        return (
          ...
          (formData.valueType === "Contact For Details" || formData.amount.trim().length > 0)
        );
      default:
        return true;
    }
  ...
  const handleDocumentsAdded = useCallback(
    (files: File[]) => {
      const accepted = files
        .filter((f) => ALLOWED_DOC_EXTS.some((ext) => f.name.toLowerCase().endsWith(ext)))
        .map<DocumentItem>((f) => ({
          id: makeId(),
          name: f.name,
          size: f.size,
  ...
  return (
    <div className="bg-cream min-h-[calc(100vh-5rem)] flex flex-col">
      ...
      <div className="flex-1 max-w-3xl w-full mx-auto px-5 md:px-10 py-8 md:py-12">
        ...
        {step === 4 && (
          <BasicsForm formData={formData} update={update} />
        )}
        ...
        {step === 6 && (
          <FinancialForm formData={formData} update={update} />
        )}
        ...
        <div className="flex gap-3 rounded-2xl bg-gold-500/10 ring-1 ring-gold-500/30 p-5">
          ...
          <div className="text-sm text-navy-900/90">
            ...
            <p className="mt-1 text-navy-900/70 leading-relaxed">
              Members will need to contact you directly to discuss numbers. This is a good choice
              for confidential acquisitions or competitive deal flow.
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 items-start rounded-xl bg-bone/60 ring-1 ring-navy-900/[0.05] px-4 py-3">
        <Info className="h-4 w-4 text-gold-600 shrink-0 mt-0.5" strokeWidth={2} />
        <p className="text-xs text-navy-700/75 leading-relaxed">
          Images will be automatically optimized for display. No need to resize before uploading.
        </p>
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2 items-start rounded-xl bg-bone/60 ring-1 ring-navy-900/[0.05] px-4 py-3">
        <Info className="h-4 w-4 text-gold-600 shrink-0 mt-0.5" strokeWidth={2} />
        <p className="text-xs text-navy-700/75 leading-relaxed">
          Documents are stored privately and only released to members after they countersign an NDA.
        </p>
      </div>
...
function SuccessScreen({ onCreateAnother }: { onCreateAnother: () => void }) {
  return (
    <div className="bg-cream min-h-[calc(100vh-5rem)] flex items-center">
      <div className="max-w-xl mx-auto px-5 md:px-10 py-16 text-center">
        ...
        <p className="mt-4 text-base text-navy-700/75 leading-relaxed">
          Our team reviews new listings within 24 hours. You&apos;ll be notified the moment yours
          is live on Capital Circle.
        </p>
